...
.whole-bar-wrap { margin-bottom: 22px; }
.whole-bar { display: flex; width: 100%; height: 40px; border-radius: 8px; overflow: hidden;
  border: 1px solid var(--border); }
.whole-bar-seg { display: flex; align-items: center; height: 100%; font-size: 12px; font-weight: 600;
  white-space: nowrap; overflow: hidden; padding: 0 10px; box-sizing: border-box; flex-shrink: 0; }
.whole-bar-seg.unclassified { background: repeating-linear-gradient(135deg, var(--gridline), var(--gridline) 6px,
  var(--surface-1) 6px, var(--surface-1) 12px); color: var(--text-secondary); justify-content: flex-start; }
.whole-bar-seg.classified { background: var(--series-1); color: #ffffff; justify-content: center; }
.whole-bar-caption { display: flex; justify-content: space-between; gap: 16px; margin-top: 8px;
  font-size: 12px; color: var(--text-secondary); line-height: 1.5; }
.whole-bar-caption .unclassified-note { max-width: 60%; }
.whole-bar-caption .classified-note { max-width: 36%; text-align: right; }
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <p class="desc">A 2,000,000-read subsample of the reads that didn't align to the <em>C. arabica</em> reference, classified against Kraken2's PlusPF-8 database (bacteria, archaea, viruses, protozoa, fungi, human).</p>
    <div class="whole-bar-wrap">
      <div class="whole-bar" id="whole-bar"></div>
      <div class="whole-bar-caption">
        <span class="unclassified-note" id="unclassified-note"></span>
        <span class="classified-note" id="classified-note"></span>
      </div>
    </div>
    <p class="desc">Bars below show the species/genus-level taxa Kraken2 could actually pin down, within that classified slice — the higher ranks above them in the tree (Bacteria, Pseudomonadota, ...) are cumulative parents of these and are left out here to avoid double-counting.</p>
    <div class="legend">
...
(function(){
  ...
  (function wholeBarChart(){
    const b = data.broad;
    const classifiedPct = 100 - b.unclassified_pct;
    const wrap = document.getElementById('whole-bar');

    const unclassifiedSeg = document.createElement('div');
    unclassifiedSeg.className = 'whole-bar-seg unclassified';
    unclassifiedSeg.style.width = b.unclassified_pct + '%';
    unclassifiedSeg.textContent = 'Unclassified — ' + b.unclassified_pct.toFixed(1) + '%';
    wrap.appendChild(unclassifiedSeg);
    unclassifiedSeg.addEventListener('pointerenter', function(evt){
      showTip(evt, ['Unclassified', fmt(b.unclassified_reads,0) + ' of ' + fmt(data.subsample_n,0) + ' reads', 'No confident match to anything in PlusPF-8 — not "empty," just not matched to this database.']);
    });
    unclassifiedSeg.addEventListener('pointermove', positionTip);
    unclassifiedSeg.addEventListener('pointerleave', hideTip);

    const classifiedSeg = document.createElement('div');
    classifiedSeg.className = 'whole-bar-seg classified';
    classifiedSeg.style.width = classifiedPct + '%';
    classifiedSeg.textContent = classifiedPct.toFixed(1) + '%';
    wrap.appendChild(classifiedSeg);
    classifiedSeg.addEventListener('pointerenter', function(evt){
      showTip(evt, ['Classified', fmt(data.subsample_n - b.unclassified_reads,0) + ' of ' + fmt(data.subsample_n,0) + ' reads', 'Matched a taxon in PlusPF-8 -- broken down by species/genus below.']);
    });
    classifiedSeg.addEventListener('pointermove', positionTip);
    classifiedSeg.addEventListener('pointerleave', hideTip);

    document.getElementById('unclassified-note').textContent =
      'Unclassified means no confident match to anything in this database -- not that the read is empty or junk. PlusPF-8 has no plant genomes at all, so any real coffee DNA in this pool would land here too, structurally, regardless of how well-preserved it is. The dedicated coffee-only database to the right is what actually tests for that.';
    document.getElementById('classified-note').textContent =
      'The ' + classifiedPct.toFixed(1) + '% Kraken2 could pin down, broken into species/genus below.';
  })();
